...
Repository contains Godot 4.2 project with XR and main scene preconfigured. Releases are reduced to addon files only. Executable binaries are included in latest release page for Windows (exe) and Quest (apk).
...
General project license is MIT, unless LICENSE file exists in a folder, which then overrides license for all files inside this folder.
